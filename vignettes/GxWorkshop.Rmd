...
When studying Pharmacogenomic in vitro, the design of the experiment usually involves doing two things in parallel:
...
NA values. For the molecular profiles, the data are returned in a `r BiocStyle::Biocpkg("SummarizedExperiment")`object, while drug sensitivity
data are returned as a matrix.
...
targetted therapies, from the GDSC data. We will do this with the first (largely random) 100 genes measured in the 
expression matrix, as the full computation can take some time to complete.

```{r drugSensSigEx, eval=TRUE, echo=TRUE}

## Extracting the first 100 gene names. featureInfo brings up the rowData of the "rna" SummarizedExperiment
chosen.genes <- rownames(featureInfo(GDSC, "rna"))[100]
sigs <- drugSensitivitySig(GDSC, "rna", sensitivity.measure = "aac_recomputed", drugs=c("Erlotinib", "Lapatinib"), features = chosen.genes)
