...
dose-response experiments for this dataset and is analagous to the drug slot
...
RadioGx provides an interface similar to other BHKLab packages for accessing
datasets we have curated.
...
As the field of Radio-genomics is relatively new, there is only one dataset 
...
Similar to PharmacoGx and Xeva, a summary of the contents of the RadioSet is printed when calling a RadioSet.

### Accessing Data

* Access the molecular features, dose-response and metadata contained within the data structures defined in the packages

```{r molecular_features}
...
response data. This package provides a function for fitting linear-quadratic
models to dose response data. The functions can be used with data contained in
a RadioSet or with raw data.
