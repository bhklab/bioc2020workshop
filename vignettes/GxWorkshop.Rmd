...
The first two of the packages, `r BiocStyle::Biocpkg("PharmacoGx")` and `r BiocStyle::Biocpkg("RadioGx")` focus on screens done  using cell lines as a
...
treatment for irradiation of cell lines. The third package,  `r BiocStyle::Biocpkg("Xeva")`, looks at treatment response for _in vivo_ systems,
...
RadioGx and PharmacoGx are both used to analyze data from in vitro, cell based assays.
In cancer, the most common in vitro model used is the cancer cell line. These cell lines
...
When studying Pharmacogenomics in vitro, the design of the experiment usually involves doing two things in parallel:
...
Currently, there is only one major, high-throughput dataset looking at _in vitro_ radiogenomics,
...
Patent derived xenografts (PDXs) are commonly used in vivo models in cancer research.
...
Once established, tumors from the PDXs can be passed from mouse to mouse, this is called passaging. For drug testing, PDXs were given a set drug dose at a set schedule (for example weekly). Change in tumor size is measured, usually physically using calipers.
![](PDXfigure500ppi.png)
...
<!-- TODO:: insert a class diagram for Xeva? -->
...
This also means that a lot of the functions used to access the data provided in these 
objects can be accessed using the same interface. 


In general, the PSet and RSet store three major categories of data: 
metadata/annotations, molecular data and radiation response data.
...
`drug` in PharmacoGx, and `radiation` in RadioGx. They are discussed just below. 
...
Data for treatment response can also be accesssed the same way across `PharmacoGx` and `RadioGx`.
...
As an exercise, lets manually compute the number of overlaping cell lines between CCLE and GDSC. Try it yourself! 
...
The structure of Xeva is different from the CoreGx structure. As such, many of the accessor functions
are also different.
...
$E_\infty$, which is the maximal inhibition predicted at infinite concetration of the drug (the right asymptote);
...
inhibition/binding of the target for the small molcule ligands, which values less than 1 imply negative cooperativity/antagonism. 
...
Drug response in PDX models is usually not assessed in the same way as in vitro. 
Rather than looking at viability vs dose curves, one looks at tumour growth vs
time curves, to see if the treatement with a drug can inhibit tumour growth or
...
```{r compute_molecular_signature, cache=TRUE}
radSensSig <- radSensitivitySig(Cleveland, mDataType='rna', nthread=2)
```
...
Below, we will use this functionality of Xeva to see if an association discovered in vitro holds up in an in vivo setting.
...
with response to treatment, in the in vitro model systems. A natural question to ask is how the signature 
...
radSig <- radSensitivitySig(Cleveland, mDataType = "rna", features=ft.rad)

gdscSigs <- drugSensitivitySig(GDSC, mDataType="rna", sensitivity.measure="aac_recomputed", drugs = drugs.selected, features=ft.drug)
```


We will preload the precomputed signatures in interest of computation time. 
```{r sig_load}
data(gdscSigs)
data(radSig)
...
with the weights derived from significance estimates for the association of each gene with drug response.  
...
ERBB2 <- PharmacoGx::drugSensitivitySig(GRAY, mDataType="Kallisto_0.46.1.rnaseq", drug="Trastuzumab", sensitivity.measure="aac_recomputed", feature="ENSG00000141736.13")[1,1,]
ERBB2.in.vivo <- drugSensitivitySig(brca, "trastuzumab", mDataType = "RNASeq", features=c("ESR1", "ERBB2"), sensitivity.measure = "best.average.response")
